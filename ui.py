...
class UI:
    def __init__(self, player):
        ...
        self.character_icons = {
            'Mage': pygame.image.load('graphics/icons/char_portrait/Mage.jpg'),
            'Fighter': pygame.image.load('graphics/icons/char_portrait/Fighter.jpg'),
            'Rogue': pygame.image.load('graphics/icons/char_portrait/Rogue.jpg'),
            'Priest': pygame.image.load('graphics/icons/char_portrait/Priest.jpg'),
        }
    ...
    def draw_stats(self):
        ...
        # Draw health bar
        pygame.draw.rect(self.display_surface, 'red', (4,  4, stats_bar_lenght * health_ratio, stats_bar_height))
        # Draw health label
        ...
        self.display_surface.blit(health_label, (5, 4))

        # Draw mana bar
        pygame.draw.rect(self.display_surface, 'blue', (4, 10, stats_bar_lenght * mana_ratio, stats_bar_height))
        # Draw mana label
        ...
        self.display_surface.blit(mana_label, (5, 10))

        # Draw stamina bar
        pygame.draw.rect(self.display_surface, 'green', (4, 10, stats_bar_lenght * stamina_ratio, stats_bar_height))
        # Draw stamina label
        ...
        self.display_surface.blit(stamina_label, (10, 16))
    ...
    def draw_stats_inv(self):
        ...
        stats = {
            ...
            "Speed": (int(self.player.actual_speed * 3), int(self.player.max_speed * 3), 'purple'),
        ...
        for i, (stat_name, (actual, max_value, color)) in enumerate(stats.items()):
            ...
            # Draw the stat bar
            pygame.draw.rect(self.display_surface, color, (4, bar_start_y + i * (stats_bar_height + 5), stats_bar_length * ratio, stats_bar_height))
            
            # Draw the label
            stat_text = f"{stat_name}: {int(actual)}/{int(max_value)}"
            stat_label = pygame.font.Font(None, 12).render(stat_text, True, 'white')
            self.display_surface.blit(stat_label, (16, bar_start_y + i * (stats_bar_height + 5)))


    def draw_game_menu(self, recruitable_characters, rooster_characters, game_stage):
        # Draw recruitable characters grid (bottom left)
        grid_start_x = 4
        grid_start_y = HEIGHT - 34  # Adjust as needed
        ...
        for i, character in enumerate(recruitable_characters):
            if character_image := self.character_icons.get(character.player_class):
                # Calculate position
                pos_x = grid_start_x + (i % 5) * (16 + 4)
                pos_y = grid_start_y + (i // 5) * (16 + 4)
                self.display_surface.blit(character_image, (pos_x, pos_y))

                # Check for mouse click
                if pygame.mouse.get_pressed()[0]:  # Left mouse button
                    mouse_pos = pygame.mouse.get_pos()
                    if pos_x <= mouse_pos[0] <= pos_x + 16 and pos_y <= mouse_pos[1] <= pos_y + 16:
                        self.transfer_character(character, recruitable_characters, rooster_characters)

        # Draw rooster characters grid (top right)
        grid_start_x_rooster = WIDTH - 64  # Adjust as needed
        grid_start_y_rooster = 4  # Adjust as needed

        for i, character in enumerate(rooster_characters):
            if character_image := self.character_icons.get(character.player_class):
                self.display_surface.blit(character_image, (grid_start_x_rooster + (i % 5) * (16 + 10), grid_start_y_rooster + (i // 5) * (50 + 10)))
    ...
    def draw_map_stage(self, selected_character, rooster_characters, game_stage):
        ...
        grid_start_x = WIDTH - 68  # Adjust as needed
        grid_start_y = 4  # Adjust as needed
    ...
    def draw_start_button(self, next_stage, actual_stage):
        ...
        button_y = HEIGHT - 68  # Adjust as needed
        button_color = 'blue'
